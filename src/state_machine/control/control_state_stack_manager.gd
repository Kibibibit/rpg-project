extends Control
class_name ControlStateStackManager

var started: bool = false


func get_current_state() -> ControlStateNode:
	if get_child_count() == 0:
		return null
	return get_child(-1) as ControlStateNode
...
func begin():
	...
	if not get_current_state():
		push_error("Control StateStackManager has no initial state!")
		return
	_connect_signals()
	get_current_state().enter()


func push_state(p_state: ControlStateNode) -> void:
...
func pop_state_until(p_name: String) -> void:
	...
	while get_current_state().name != p_name:
		if not get_current_state():
			push_error("Control Never found a state with name %s" % p_name)
			return
		get_current_state().exit()
		get_current_state().queue_free()
		remove_child(get_current_state())
